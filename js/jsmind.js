    ...
    jm.mind.prototype = {
        get_node:function(nodeid){
            if(nodeid in this.nodes){
                ...
                //logger.warn('the node[id='+nodeid+'] can not be found'); //yoshioka エラー数が膨大になるので一旦コメント化
                return null;
            }
        },
        ...
        remove_node:function(node){
            ...
            while(ci--){
                ...
                //yoshioka 子ノードの削除も記録されること

                console.log(children[ci].id);

                var asasda = Get_NodeInfo(children[ci].id, "parent_id");
                var adeewf = Get_NodeInfo(children[ci].id, "concept_id");
                var wefwef = Get_NodeInfo(children[ci].id, "type");

                Record_activities(children[ci].id,
                                  asasda,
                                  "delete",
                                  "",
                                  adeewf,
    ...
    jm.format = {
        ...
        node_array:{
            ...
            _parse:function(mind, node_array){
                var df = jm.format.node_array;
                // 引用：arr.slice(0)は、イデックスがゼロのところから残り全部を切り出すということで、これは単にオブジェクトを複製しているだけらしい。
                // 引用：配列っぽいけど配列じゃないargumentsなどのオブジェクトを配列に変換するなどの目的
    ...
    jm.prototype={
            ...
            if(!!nodeid){
                ...
                if(event.shiftKey == false){//シフトキーを押していない時
                    ...
                    for(i=0; i<jmnode.length; i++){
                        ...
                        }else if(jmnode[i].getAttribute("type") == "toi"){//問いノードの時
                    ...
                    for(i=0; i<jmnode.length; i++){
                        ...
                        if(jmnode[i].getAttribute("type") == "answer"){
                ...
                for(i=0; i<jmnode.length; i++){
                    ...
                    if(jmnode[i].getAttribute("nodeid") == thisId){
                        ...
                        //クリックしたノードが答えノードだった場合，合理性を考えるべき問いがマップ上にあるか探索
            ...
            if(this.get_editable()){
                ...
                if(!!node){
                  ...
                  Record_activities(nodeid,
                                    Get_NodeInfo(nodeid, "parent_id"), //親ノードIDを返す関数
                                    "edit",
                                    topic,
                                    Get_NodeInfo(nodeid, "concept_id"), //concept_idを取得
                                    Get_NodeInfo(nodeid, "type"), //タイプを取得
                                    jsMind.util.uuid.newid()
                                   );
        ...
        move_node:function(nodeid, beforeid, parentid, direction){
            ...
            for(var j=0; j<jmnode.length; j++){
                ...
                if(parentid == jmnode[j].getAttribute("nodeid")){

                    var n_concept = jmnode[j].getAttribute("concept_id");

                }

            }

            //移動したノードが答えノードなら，移動後の親ノードの概念IDを取得
            //移動したノードが問いノードなら，概念IDはそのまま
            for(var i=0; i<jmnode.length; i++){
                ...
                if(nodeid == jmnode[i].getAttribute("nodeid")){
                    ...
                    if(jmnode[i].getAttribute("type") == "answer"){
                        ...
                        jmnode[i].setAttribute("concept_id",n_concept);

                    }else{

                        n_concept = jmnode[i].getAttribute("concept_id");
            ...
            //yoshioka登録　親ノードを変更したこと(ノードを移動させたこと)
            //渡す情報（ノードID，親ノードID，操作，テキスト，法造コンセプトID，タイプ，primary）
            Record_activities(nodeid,
                              parentid,
                              "move",
    ...
    jm.shortcut_provider.prototype = {
        ...
        handle_addchild: function(_jm,e){
            ...
            if(!!selected_node){
                ...
                var versionid = jm.util.uuid.newid();   //hatakeyama
                ...
                for(var i=0; i<jmnode.length; i++){
                    ...
                    if(selected_node.id == jmnode[i].getAttribute("nodeid")){

                        var p_type = jmnode[i].getAttribute("type");
                ...
                for(var j=0; j<jmnode.length; j++){
                    ...
                    if(nodeid == jmnode[j].getAttribute("nodeid")){
                        ...
                        jmnode[j].setAttribute("parent_id",selected_node.id);
                        ...
                        // hatakeyama Enterキーで答え/問いノードを追加
                        NodeInsert(
                            versionid, 
                            nodeid, 
                            selected_node.id, 
                        ...
                        //yoshioka登録　Enterキーより答え/問いを追加したこと
                        //渡す情報（ノードID，親ノードID，操作，テキスト，法造コンセプトID，タイプ，primary）
                        Record_activities(nodeid,
                                          selected_node.id,
                                          "add",
        ...
        handle_addbrother:function(_jm,e){
            ...
            if(!!selected_node && !selected_node.isroot){
                var nodeid = jm.util.uuid.newid();
                var versionid = jm.util.uuid.newid();   //
                ...
                for(var i=0; i<jmnode.length; i++){
                    ...
                    if(selected_node.id == jmnode[i].getAttribute("nodeid")){
                        ...
                        var parent_id = jmnode[i].getAttribute("parent_id");
                ...
                for(var j=0; j<jmnode.length; j++){
                    ...
                    if(parent_id == jmnode[j].getAttribute("nodeid")){

                        var p_type = jmnode[j].getAttribute("type");
                        var p_concept = jmnode[j].getAttribute("concept_id");
